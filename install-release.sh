...
get_latest_version() {
  ...
  local tmp_file
  tmp_file="$(mktemp)"
  if ! curl -x "${PROXY}" -sS -H "Accept: application/vnd.github.v3+json" -o "$tmp_file" 'https://api.github.com/repos/hiddify/Xray-core-custom/releases/latest'; then
    "rm" "$tmp_file"
  ...
  if ! curl -x "${PROXY}" -sS -H "Accept: application/vnd.github.v3+json" -o "$tmp_file" 'https://api.github.com/repos/hiddify/Xray-core-custom/releases'; then
    "rm" "$tmp_file"
    echo 'error: Failed to get release list, please check your network.'
    exit 1
  fi
  ...
  do
    ...
    grep -q "https://github.com/hiddify/Xray-core-custom/releases/download/${releases_list[$i]}/Xray-linux-$MACHINE.zip" "$tmp_file" && break
  done
  "rm" "$tmp_file"
...
download_xray() {
  DOWNLOAD_LINK="https://github.com/hiddify/Xray-core-custom/releases/download/${INSTALL_VERSION}/Xray-linux-${MACHINE}.zip"
